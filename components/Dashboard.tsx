...
import { TrendingUp, DollarSign, ShoppingBag, Store, Table, BarChart3, ArrowDownWideNarrow } from 'lucide-react';
...
const MatrixTable = ({ title, rows, cols, data, rowKey, colKey, valueKey, showTotal = true }: any) => {
  // Calculate Totals
...
export const Dashboard = () => {
  ...
  const matrixData = useMemo(() => {
    ...
    filtered.forEach(item => {
      ...
      const monthIdx = new Date(item.date).getMonth(); // 0-11
    ...
    // Sort products by total volume desc
      ...
      {activeTab === 'charts' ? (
        ...
        <div className="animate-fade-in space-y-8">
            {matrixData && (
                <>
                    <MatrixTable 
                        title="1. Reporte de Ventas por Tienda y Línea" 
                    ...
                    <MatrixTable 
                        title="2. Venta de Tableros por Meses (Evolución Tiendas)" 
                    ...
                    <MatrixTable 
                        title="3. Venta Detallada por Producto y Mes" 
